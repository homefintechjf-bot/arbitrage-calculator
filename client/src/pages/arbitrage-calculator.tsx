...
export default function ArbitrageCalculator() {
  ...
  const showPredictItWarning = useMemo(() => {
    const names = [siteAName.toLowerCase(), siteBName.toLowerCase(), siteCName.toLowerCase()];
    return names.some(name => name.includes("predictit"));
  }, [siteAName, siteBName, siteCName]);
  ...
  const results = useMemo((): ArbitrageResult[] | null => {
    ...
    const validPrices = [priceA, priceB, priceC].filter(p => p !== null && p < 1);
    if (validPrices.length < 2) return null;

    // Calculate results for each valid pair
  ...
  }, [siteAName, siteBName, siteCName, siteAYes, siteBYes, siteCYes, investment, orderMode]);
  ...
  const filledSites = [siteAYes, siteBYes, siteCYes].filter(p => p !== "").length;
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-12">
        ...
        <p className="text-xs text-muted-foreground text-center mb-4">
          Fill in at least 2 sites to calculate. All 3 sites will compare every pair (A-B, A-C, B-C).
        </p>
